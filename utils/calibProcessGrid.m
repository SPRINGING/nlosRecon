%% 将实际psf和生成psf进行通过求最大相似度进行校准,
% plotFlag: 最终结果可视化
% testFlag: true:岁校准进行初步调整. false: 对校准范围进行生成
% 校准参数: objctPos = [0.60, 0.07, 0.47], 第一个参数控制第一排的左右偏移,第二个参数控制第二排的左右偏移,第三个参数控制



btchFile = 'DynS/dynS';
saveBtchFile = 'StN2nGrid1';
plotFlag = false;
testFlag = false; 
plotFlag2 = false;
...
load('resizeData/DynS/dynS_data_VidSeq_1.mat', 'vidSeq');
% load('resizeData/DynS/denoised.mat', 'denoise');
...
for i = 1:size(mapPos, 1)
	...
	% nlosReshapeData = squeeze(denoise(90, :, :, :, :));
	nlosReshapeData = squeeze(vidSeq(:, :, :, 1));
	% psfFileName = sprintf('resizeData/%s_%s.mat', btchFile, 'data1');
	% load(psfFileName, 'nlosReshapeData');
	...
	objctPos = [0.44, 0.03, 0.54] + anchorPos + constOffset;
	normalization = false;
	sig = 0.05;
